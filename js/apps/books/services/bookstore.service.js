...
function query(filterBy) {
  ...
  if (!books) {
    _saveToStorage(booksList);
    books= _loadFromStorage()
  }
  if (filterBy) {
    ...
    books = books.filter(book => {
      ...
      return(book.title.toLowerCase().includes(name.toLowerCase()) && book.listPrice.amount <= price);
    });
  }
  // console.log(books)
...
function addReview(id, review){
  ...
  const bookToUpdate = books[bookIdx]
  if (!bookToUpdate.review) bookToUpdate.review = []
  bookToUpdate.review.push(review)
...
function deleteReview(bookId,revId){
  ...
  const bookToUpdate = books[bookIdx]
  const revIdx=bookToUpdate.review.findIndex(bookRev => bookRev.readAt === revId)
  bookToUpdate.review.splice(revIdx, 1)
  _saveToStorage(books)
  return Promise.resolve(bookToUpdate)
}

function _getBookIdxById(id){
  let books = _loadFromStorage();
  let bookIdx = books.findIndex(book => book.id === id)
...
function addGoogleBook(book){
  let {title,subtitle,authors,publishedDate,description,pageCount,categories,imageLinks, language} = book
  ...
  let newBook = {
      ...
      listPrice: {
        ...
        currencyCode: (utilService.getRandomIntInclusive(10,200)%2 ===0)?"EUR":"ILS",
        isOnSale: (utilService.getRandomIntInclusive(10,200)%2 ===0)?true:false
      }
  }
